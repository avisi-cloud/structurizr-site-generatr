...
class CDN(val workspace: Workspace) {
    private val cdnBaseURL = workspace.views.configuration.properties.getOrDefault("generatr.site.cdn", "https://cdn.jsdelivr.net/npm")
    ...
    data class Dependency(val cdnBaseURL: String, val name: String, val version: String) {
        fun baseUrl() = when (cdnBaseURL.last().toString()) {
                        "/" -> "$cdnBaseURL$name@$version"
                        else -> "$cdnBaseURL/$name@$version"
        }
    }
}
